...
{
    private array $chart;
    private ChartConfigResolver $config;

    public function __construct($type)
    {
        $this->config = app(ChartConfigResolver::class);
        $this->chart = $this->config->provider($type);
    }
    ...
    public function resolve(string $groupByField, array|Collection $data = [], ?string $chartType = 'all'): array
    {
        ...
        if ($chartType === 'all') {
            ...
            $types = $this->config->supportedTypes($this->chart);
    ...
    private function formatStanderSeries(string $groupByField, array $data): array
    {
        ...
        foreach ($data as $item) {
            foreach ((array)$item as $k => $v) {
                if ($k === $groupByField) {
                    $categories[] = rb_resolve_trans($v);
                } elseif (!Str::endsWith($k, '_id')) {
                    $series[$k]['name'] = rb_resolve_trans($k);
                    $series[$k]['data'][] = $v;
                }
            }
        }
    ...
    {
        $chart = $this->config->chart($this->chart, $chartType);
        $chart['xAxis']['categories'] = $categories;
        $chart['series'] = collect(array_values($series))->map(function ($item) {
            $item['color'] = $this->config->primaryColor();
            return $item;
        })->toArray();
    ...
    {
        $chart = $this->config->chart($this->chart, 'pie');
        $chart['series'][] = [
            'name' => rb_resolve_trans('count'),
            'colorByPoint' => true,
            'data' => collect($series)
                ->map(fn($item) => [
                    'name' => rb_resolve_trans($item['name']),
